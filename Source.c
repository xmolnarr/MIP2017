...
void uprav(struct vetva* succ, struct vetva* odstran)
{
	succ->lavy = odstran->lavy;
	if (succ->lavy != NULL) succ->lavy->rodic = succ;
	succ->vyskaL = odstran->vyskaL;
	succ->rodic = odstran->rodic;

	if (succ != odstran->pravy)
	{
		succ->pravy = odstran->pravy;
		odstran->pravy->rodic = succ;
	}
}
